...
sys_symlink(){
  ...
  // check if arguments were as expected
  ...
  // create inode  on path of type T_SYMLINK
  struct inode *i = create(path, T_SYMLINK, 0,0);
  ...
  // i must be locked while write but is already locked from create
  int m = writei(i, 0, (uint64)target, 0, n);

  // if not whole length will be written
  if (m!=n){
    end_op();
    return -1;
  }
